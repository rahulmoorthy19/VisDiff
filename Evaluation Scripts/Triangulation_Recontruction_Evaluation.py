...
def dual_generation(predicted_visibility, actual_dual):
    dual = np.zeros((actual_dual.shape[0], actual_dual.shape[1]))
    for i in range(actual_dual.shape[0]):
        for j in range(actual_dual.shape[1]):
            if actual_dual[i,j] == 1.0 and predicted_visibility[i,j] == 1.0:
                dual[i,j] = 1.0
    return dual
...
def ordering_points_get_visibility(points, target_visibility_graph):
    vis_est = Visibility_Calculator().visibility_matrix_generator(points)
    best_ordering = None 
    best_accuracy = -1
    best_visibility = None

    for each_vertex in range(points.shape[0]):
        current_ordering = list()
        current_ordering_start = copy.deepcopy(each_vertex)
        current_ordering.append(current_ordering_start)
        current_ordering_next = (copy.deepcopy(each_vertex) + 1) % points.shape[0]
        while current_ordering_next !=  current_ordering_start:
            current_ordering.append(current_ordering_next)
            current_ordering_next = (copy.deepcopy(current_ordering_next) + 1) % points.shape[0]
            if current_ordering_next ==  current_ordering_start:
                break
        current_ordering_visibility = np.ones(target_visibility_graph.shape)
        for each_order in range(len(current_ordering)):
            for each_order_next in range(len(current_ordering)):
                current_ordering_visibility[each_order, each_order_next] = vis_est[current_ordering[each_order], current_ordering[each_order_next]]
                current_ordering_visibility[each_order_next, each_order] = vis_est[current_ordering[each_order_next], current_ordering[each_order]]
        current_accuracy = f1_score(target_visibility_graph.flatten(), current_ordering_visibility.flatten(), average = "macro")
        if best_accuracy < current_accuracy:
            best_accuracy = copy.deepcopy(current_accuracy)
            best_visibility = copy.deepcopy(current_ordering_visibility)
            best_ordering = copy.deepcopy(current_ordering)
    ...
    return points[np.array(best_ordering)], best_visibility, best_accuracy
...
for each_file in os.listdir(test_dataset):
    if os.path.isfile(test_dataset + each_file):
        ...
        final_level_2_visibility = polygon_dict["triangulation_graph"]
        ...
        generated_sdf_batch = samples(sdf_generator, visibility_encoded_batch, sigmas, 1.0, 0.5, None, no_samples)
        ...
        for each_polygon in range(final_vertices_generated_batch_all.shape[0]):
            ...
            predicted_visi = FastVisibilityCalculator().visibility_matrix_generator(current_polygon)
            predicted_visibility = dual_generation(predicted_visi, final_level_2_visibility)
        ...
        current_file["predicted_dual"] = predicted_visibility
        current_file["true_polygon"] = polygon_dict["level_2_polygon"]
        current_file["true_dual"] = polygon_dict["triangulation_graph"]
...
with open('Visdiff_Predictions_Visibility_Final_Ordering_Diffusion_Pixel_Aligned_Global_Dual.pkl', 'wb') as f:
