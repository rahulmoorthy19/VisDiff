...
for each_file in os.listdir(test_dataset):
    if os.path.isfile(test_dataset + each_file):
        ...
        if int(each_file.split("_")[1]) > 200:
            actual_validity.append(1)
            current_validity = 1
        else:
            actual_validity.append(0)
            current_validity = 0
        ...
        for each_polygon in range(final_vertices_generated_batch_all.shape[0]):
            ...
            predicted_visibility = Visibility_Calculator().visibility_matrix_generator(current_polygon)
        ...
        current_file["visibility_validity"] = current_validity
        evaluation_plots[each_file] = current_file
        # Visualization
        cnt += 1
        
## Saving the predictions for quantitative calulations
with open('Visdiff_Predictions_Visibility_Final_Ordering_Diffusion_Pixel_Aligned_Global_Recognition.pkl', 'wb') as f:
