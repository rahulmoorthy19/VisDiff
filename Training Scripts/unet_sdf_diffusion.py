...
class UNet_SD(nn.Module):
    ...
    def __init__(self, in_channels=4,
        ...
        self.visibility_embedding = nn.Sequential(OrderedDict({
            "linear_1": nn.Linear(325, context_dim, bias=True),
            "act": nn.SiLU(),
            "linear_2": nn.Linear(context_dim, context_dim, bias=True),
        })
        ) 
    ...
    def forward(self, x, time_steps, visibility=None, encoder_hidden_states=None, output_dict=True):
        if visibility is None and encoder_hidden_states is not None:
            cond = encoder_hidden_states
        t_emb = self.time_proj(time_steps)
        t_emb = self.time_embedding(t_emb)

        cond = self.visibility_embedding(visibility).unsqueeze(1)
        x = self.conv_in(x)
        down_x_cache = [x]
        ...
        for module in self.up_blocks:
            feature = torch.cat((x, down_x_cache.pop()), dim=1)
            x = module(feature , t_emb, cond)
        x = self.output(x)
        if output_dict:
...
class TransformerBlock(nn.Module):
    def __init__(self, hidden_dim, context_dim, num_heads=2):
...
class SpatialTransformer(nn.Module):
    def __init__(self, hidden_dim, context_dim, num_heads=2):
        ...
        self.transformer_blocks = nn.Sequential(
            TransformerBlock(hidden_dim, context_dim, num_heads=2)
        )  # to be compatible with Diffuser, could simplify.
